...
from app.utils.cost_tracker import LLMProvider, cost_tracker
...
class LLMCostService:
    ...
    def __init__(self) -> None:
        ...
        self.daily_budget_usd: float | None = None
    ...
    def get_daily_cost(self, date: datetime | None = None) -> float:
        ...
        daily_metrics = [
            m for m in self.cost_tracker.metrics if start_of_day <= m.timestamp < end_of_day
        ]
    ...
    def generate_report(
        ...
        period_metrics = [
            m for m in self.cost_tracker.metrics if start_time <= m.timestamp <= end_time
        ]
        ...
        for metric in period_metrics:
            ...
            cost_by_provider[provider] = cost_by_provider.get(provider, 0.0) + metric.cost_usd

            model_key = f"{provider}:{metric.model}"
            cost_by_model[model_key] = cost_by_model.get(model_key, 0.0) + metric.cost_usd
...
_llm_cost_service: LLMCostService | None = None
